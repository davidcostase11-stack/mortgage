...
# --- Tabs ---
tab1, tab2 = st.tabs(["📊 Simulador de Crédito", "📅 Projeções de Mercado (BCE)"])

with tab1:
    # --- 1. Base Inputs ---
    st.markdown('<div class="section-header">Dados do Empréstimo Atual</div>', unsafe_allow_html=True)
    i_col1, i_col2, i_col3, i_col4 = st.columns(4)
    with i_col1:
        debt = st.number_input("Dívida Atual (€)", value=150000, step=1000, key="debt_in")
    with i_col2:
        months = st.number_input("Meses Restantes", value=360, step=12, key="months_in")
    with i_col3:
        spread = st.number_input("Spread (%)", value=1.0, step=0.05, format="%.2f", key="spread_in")
    with i_col4:
        actual_euribor = st.number_input("Euribor Atual (%)", value=2.0, step=0.01, format="%.2f", key="euribor_in")

    current_rate = spread + actual_euribor
    p_baseline = calculate_mortgage(debt, current_rate, months)

    # --- 2. Custom Simulation ---
    st.markdown('<div class="section-header" style="margin-top:30px;">Simulação Personalizada</div>', unsafe_allow_html=True)
    s_col1, s_col2 = st.columns(2)
    with s_col1:
        new_euribor = st.number_input("Nova Euribor Prevista (%)", value=2.5, step=0.01, format="%.2f", key="new_euribor_in")
    with s_col2:
        repay_amt = st.number_input("Valor a Amortizar (€)", value=10000, step=500, key="repay_in")
...
with tab2:
    st.markdown('<div class="section-header">Projeções do Mercado Financeiro</div>', unsafe_allow_html=True)
    st.info("ℹ️ Estes valores baseiam-se nos alvos mais prováveis das próximas reuniões do Banco Central Europeu (BCE).")
    ...
    p_cols = st.columns(4)
    # Recalculate baseline for tab2 context
    current_rate_tab2 = spread + actual_euribor
    p_baseline_tab2 = calculate_mortgage(debt, current_rate_tab2, months)

    for i, proj in enumerate(projections):
        with p_cols[i]:
            p_rate = spread + proj['rate']
            p_val = calculate_mortgage(debt, p_rate, months)
            p_diff = p_val - p_baseline_tab2
